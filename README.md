...
| `repair_parts`           | 维修配件表         |
| `customers`              | 客户表（含服务费到期时间） |
| `service_fee_records`    | 服务费记录表        |
...
- 客户管理模块功能增强
  - 将"邮箱"字段改为"服务费到期时间"
  - 新增"到期状态"列，自动显示"已到期"或"未到期"
  - 新增到期时间列，显示最后的服务费到期时间，并可点击排序
  - 操作栏添加服务费管理按钮（金钱图标）
  - 门店管理和客户管理增加唯一编码字段
  - 新增服务费记录管理功能
    - 支持添加/编辑/删除服务费记录
    - 每条记录包含：开始日期、结束日期、交款日期、是否交款、备注、到期状态
    - 自动检查时间重叠，防止创建重叠的服务费记录
    - 实时计算并显示到期状态
### v1.3.4
...
- 修复删除商品的外键约束错误
  ...
  - 执行物理删除时使用 PRAGMA foreign\_keys 控制外键约束
