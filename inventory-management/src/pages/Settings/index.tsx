...
const Settings: React.FC = () => {
  ...
  const [displayMode, setDisplayMode] = useState<'windowed' | 'fullscreen'>('windowed')
  ...
  const loadSystemSettings = async () => {
    try {
      ...
      if (response.success && response.data) {
        ...
        // 获取显示模式设置（默认为窗口化全屏）
        const savedDisplayMode = settings.displayMode || 'windowed-fullscreen'
        setDisplayMode(savedDisplayMode as 'windowed' | 'fullscreen' | 'windowed-fullscreen')
  ...
  // 切换显示模式
  const handleDisplayModeChange = async (mode: 'windowed' | 'fullscreen' | 'windowed-fullscreen') => {
    try {
      setDisplayMode(mode)
      ...
      if (response.success) {
        message.success(`已切换到${mode === 'fullscreen' ? '全屏' : '窗口'}模式，重启后生效`)
      } else {
        message.error(response.error || '保存失败')
      }
    } catch (error: any) {
      ...
      <Card>
        ...
        <Tabs 
          ...
          items={[
                    ...
                    <Form form={systemForm} layout="vertical">
                      ...
                      >
                        <Select
                          ...
                          style={{ width: 150 }}
                        >
                          <Select.Option value="windowed-fullscreen">窗口化全屏</Select.Option>
                          <Select.Option value="windowed">窗口化</Select.Option>
                          <Select.Option value="fullscreen">全屏</Select.Option>
                        </Select>
                      </Form.Item>
                    </Form>
                  </Card>

                  <Card title="更新服务器配置" style={{ marginTop: 24 }}>
                    <Form form={systemForm} layout="vertical">
                      <Form.Item
                        label="更新服务器地址"
            ...
            {
              key: 'backup',
              label: <span><DatabaseOutlined />数据备份</span>,
              children: (
                <div>
                  <Card>
                    <Form 
                      ...
                      onFinish={async (values) => {
                        ...
                        try {
                          ...
                          const settings: Record<string, string> = {
                            autoBackup: String(values.autoBackup !== false),
